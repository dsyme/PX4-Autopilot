...
Run 57 added a dedicated correspondence section for `CommanderArming.lean` (20 theorems,
0 sorry, abstraction level), completing coverage for all 21 active Lean files (27 C++
targets). Prior sections audited — no source drift detected.

The count is now **21 Lean files covering 27 C++ targets**:
- Run 56 added `CommanderArming.lean` (Commander arm/disarm FSM): now has full section.
- All previous files confirmed accurate against current source tree.

### Run 55 Review Notes

Run 55 added a dedicated correspondence section for `Hysteresis.lean` (20 theorems + 6
examples, 0 sorry). `Crc16Fold.lean` (run 51) and `SignFromBoolSq.lean` (run 49) were
audited and confirmed accurate — no source drift detected.
...
## `FVSquad/CommanderArming.lean` — Commander Arming FSM

**C++ source**: `src/modules/commander/Commander.cpp` (lines 584–730)
**Functions**: `Commander::arm(arm_disarm_reason_t, bool)`,
`Commander::disarm(arm_disarm_reason_t, bool)`
**Informal spec**: `formal-verification/specs/commander_arming_fsm_informal.md`

### What is modelled

The Lean model captures the **pure state-machine logic** of the Commander arming FSM.
The two C++ methods return a `transition_result_t` (`CHANGED`, `NOT_CHANGED`, or
`DENIED`) and write the new arming state.  The model represents:

| Lean definition | What it models |
|-----------------|---------------|
| `ArmState` | `vehicle_status_s::arming_state` (DISARMED / ARMED) |
| `ArmResult` | `transition_result_t` (DENIED / NOT_CHANGED / CHANGED) |
| `ArmConditions` | All boolean guards extracted from the C++ conditionals |
| `armFSM` | Pure functional model of `Commander::arm` |
| `disarmFSM` | Pure functional model of `Commander::disarm` |

The 9 `ArmConditions` booleans collapse the C++ guard tree:

| Lean field | C++ condition |
|------------|--------------|
...
| `mc_manual_thrust` | rotary-wing ∧ manual ∧ non-climb-rate mode |
| `rc_disarm_ok` | `COM_DISARM_MAN` parameter |

### Correspondence level: **Abstraction**

The model faithfully captures the branching structure of the arming/disarming decision
tree.  All state-machine outputs are exact for the modelled inputs.

### Divergences

| Aspect | C++ behaviour | Lean model | Impact on proofs |
|--------|--------------|-----------|-----------------|
| Side effects | Logging, MAVLink events, sound, home-position setting, `armed_time` | **Omitted** | Theorems only cover state and result; side-effect correctness not verified |
| Time / grace period | `armed_time`, `last_disarmed_timestamp`; 5 s re-arm grace period | `run_preflight_checks` bool | Grace period logic subsumed; theorems hold for both branches |
| Guard complexity | Multi-level C++ conditions with dynamic runtime values | 9 static booleans | Model is complete for the Boolean abstraction; no runtime type information |
| Thread safety | `_armed_state_lock` mutex | Not modelled | Concurrent arming/disarming not proved |
| `nav_state` | Runtime navigation mode enumeration | Part of `can_arm` bool | Mode-specific arming conditions verified only at the Bool level |

No divergence affects the **state-machine output correctness** of the proved theorems.
All 20 theorems are valid within the `ArmConditions` abstraction.

### Proved properties

| Theorem | Property |
|---------|----------|
| `arm_when_armed` | Arming when already ARMED → `NOT_CHANGED`; state unchanged |
| `disarm_when_disarmed` | Disarming when already DISARMED → `NOT_CHANGED`; state unchanged |
| `arm_changed_implies_armed` | `arm` returns `CHANGED` → new state is `ARMED` |
| `arm_denied_state_unchanged` | `arm` returns `DENIED` → state remains `DISARMED` |
| `arm_changed_implies_was_disarmed` | `arm` returns `CHANGED` → old state was `DISARMED` |
| `arm_not_changed_implies_was_armed` | `arm` returns `NOT_CHANGED` → old state was `ARMED` |
| `disarm_changed_implies_disarmed` | `disarm` returns `CHANGED` → new state is `DISARMED` |
| `disarm_denied_state_unchanged` | `disarm` returns `DENIED` → state remains `ARMED` |
| `disarm_changed_implies_was_armed` | `disarm` returns `CHANGED` → old state was `ARMED` |
| `disarm_not_changed_implies_was_disarmed` | `disarm` returns `NOT_CHANGED` → old state was `DISARMED` |
| `forced_disarm_always_succeeds` | Forced disarm of ARMED vehicle always returns `(DISARMED, CHANGED)` |
| `forced_disarm_ignores_landing` | Forced disarm reaches DISARMED regardless of landing state |
| `arm_denied_when_calibrating` | `arm` always `DENIED` when any calibration is active |
| `disarm_unaffected_by_calibration` | Disarm has no calibration guard |
| `arm_result_trichotomy` | `arm` returns exactly one of `CHANGED`/`NOT_CHANGED`/`DENIED` |
| `disarm_result_trichotomy` | `disarm` returns exactly one of `CHANGED`/`NOT_CHANGED`/`DENIED` |
| `arm_then_force_disarm` | After successful arm, forced disarm always reaches `DISARMED` |
| `arm_then_force_disarm_result` | After successful arm, forced disarm returns `CHANGED` |
| `arm_not_changed_state_stable` | `arm` with `NOT_CHANGED` → state identical to before |
| `disarm_not_changed_state_stable` | `disarm` with `NOT_CHANGED` → state identical to before |

All 20 theorems proved with 0 sorry. Verification: `lake build` passed with Lean 4.30.0-rc2.

### Known gaps

- **Re-arm grace period** (`run_preflight_checks = false` within 5 s of last disarm on
  RC-switch arm): modelled as a boolean input; the time-based guard is not verified.
- **Sequential arm-disarm-arm cycling**: proved compositionally (`arm_then_force_disarm`
  covers one cycle); multi-cycle liveness requires Hysteresis-style temporal reasoning.
- **Concurrent access**: the `_armed_state_lock` mutex invariants are not modelled.

---

## Summary of Correspondence Quality

**21 active Lean files, 27 C++ targets** (20 abstraction, 3 exact, 2 exact+mismatch/approximation).
