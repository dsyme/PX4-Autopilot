...
\title{Formal Verification of a UAV Flight Controller's Core Libraries with Lean~4:\\
       251 Theorems, Two Bugs, Zero Sorry in 19 Modules}
...
(no Mathlib), we verified 245 theorems across 19 Lean modules covering 23~C++
targets including signal-processing filters, rate-limiting, piecewise-linear
interpolation, online statistics, ring buffers, a time-delayed boolean
state machine, and fundamental arithmetic utilities. All proofs were mechanically checked by \texttt{lake build}. The
...
Latest workflow run:
\url{https://github.com/fsprojects/PX4-Autopilot/actions/runs/24600733577}}
\end{tcolorbox}
...
\begin{itemize}
  \item A formal verification campaign covering \textbf{23~C++ targets with Lean proofs}
        in 19~Lean modules (24~targets identified overall), yielding \textbf{245~proved
        theorems} plus 117~concrete examples.
  ...
  \item A \textbf{correspondence discipline} documenting modelling approximations and
        their impact on proof validity for all 19 Lean files.
...
pipelines) as confidence in the toolchain grew. The final set of 23~identified targets
spans six categories: (i)~mathematical utilities (\texttt{constrain}, \texttt{signNoZero},
...
All 23~targets now have Lean proofs (19~modules). One additional target
(\texttt{septentrio::buffer\_crc16} fold property) has an informal specification written
and is ready for Lean formalisation.
...
Table~\ref{tab:theorems} summarises the 19~Lean files (23~C++ targets). In total,
\textbf{251~theorem statements} were made, of which \textbf{245~are fully proved}
and \textbf{6~remain \texttt{sorry}-guarded} (all in \texttt{WrapAngle.lean},
requiring Mathlib's \texttt{Int.fract}). Additionally, \textbf{117~concrete
\texttt{example}/\texttt{\#eval} checks} serve as property-based regression tests.

\begin{table}[t]
\centering
\caption{Lean 4 verification summary by file (19 files, 23 C++ targets).
...
\texttt{SignFromBoolSq}& \texttt{signFromBool}, \texttt{sq} & 17 & 0 & Range $\{-1,1\}$, non-neg, even ✅ \\
\hline
\textbf{Total}         &                                    & \textbf{251} & \textbf{6} & \\
\bottomrule
\end{tabular}
\end{table}
...
Across 19~files, roughly 50\% of theorems were proved by \texttt{decide} or
...
libraries using Lean~4. Across 19~Lean modules and 23~C++ targets, we proved
245~theorems covering rate limiting, interpolation, filtering, statistics, data
...
once Mathlib becomes available; formalising the \texttt{septentrio::buffer\_crc16}
fold property (informal spec complete), whose key theorem follows trivially from
\texttt{List.foldl\_append}; and extending the pipeline to phase-1 targets
(atmospheric density model, Commander arming FSM); and exploring formal correspondence
proofs from the Lean model to the C++ source using Aeneas~\cite{aeneas} extraction
once the toolchain is available for C++.
