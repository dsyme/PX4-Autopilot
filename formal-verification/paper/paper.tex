...
\begin{itemize}
  \item A formal verification campaign covering \textbf{21~C++ targets with Lean proofs}
        in 18~Lean modules (23~targets identified overall), yielding \textbf{228~proved
        theorems} plus 112~concrete examples.
...
Of these, 21~targets have Lean proofs (18~modules); \texttt{signFromBool} and
\texttt{sq} have informal specifications written and are ready for Lean formalisation.
...
once Mathlib becomes available; advancing two phase-2 targets (\texttt{signFromBool}
and \texttt{sq}, both with complete informal specifications) to full Lean proofs in
the next run; and extending the pipeline to three phase-1 targets
(\texttt{crc16\_fold} fold property, atmospheric density model,
and Commander arming FSM); and exploring formal correspondence proofs from the Lean
